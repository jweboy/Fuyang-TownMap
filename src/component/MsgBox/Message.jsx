...
class Message extends Component {
  ...
  render() {
    ...
    if (msg.ScenicName === '新登中学') {
      const msgItem = schoolList.map((item, index) => {
        item = item.data;
        navigatorURL = `https://map.baidu.com/mobile/webapp/search/search/qt=s&wd=${item.ScenicName}&c=179&searchFlag=more_cate&nb_x=${item.LatY}&nb_y= ${item.LongX}&center_rank=1&center_name=${item.ScenicName}/vt=map`;
        if (item.ScenicName === '新登中学') {
